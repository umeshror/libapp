...
def test_borrows_api(client):
    ...
    member_res = client.post(
        "/members/", json={"name": "Mflow", "email": f"mf_{uuid.uuid4()}@e.com"}
    )
    ...
    response = client.get(f"/members/{member_id}/borrows/")
    assert response.status_code == 200
    data = response.json()
    assert "data" in data
    ...
    response = client.get("/borrows/overdue/")
    assert response.status_code == 200
    data = response.json()
    assert "data" in data
    assert len(data["data"]) == 0
    ...
    # 4. Return Book
    response = client.post(f"/borrows/{borrow_id}/return/")
    assert response.status_code == 200
...
def test_borrow_edge_cases_api(client):
    ...
    member_res = client.post(
        "/members/", json={"name": "Edge Mem", "email": f"edge_{uuid.uuid4()}@e.com"}
    )
    member_id = member_res.json()["id"]

    # 1. Invalid Member w/ Valid Book
    bad_mem_id = "00000000-0000-0000-0000-000000000000"
    response = client.post(f"/members/{bad_mem_id}/borrows/?book_id={book_id}")
    assert response.status_code == 404
    assert response.json()["detail"] == "Member not found."

    # 2. Valid Member w/ Invalid Book
    bad_book_id = "00000000-0000-0000-0000-000000000000"
    response = client.post(f"/members/{member_id}/borrows/?book_id={bad_book_id}")
    assert response.status_code == 404
    ...
    response = client.post(f"/members/{member2_id}/borrows/?book_id={book_id}")
